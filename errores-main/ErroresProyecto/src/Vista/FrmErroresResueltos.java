...
public class FrmErroresResueltos extends JFrame {
    ...
    private final transient ErrorDAO errorDAO;
    ...
    private void filtrar() {
        ...
        // Validar formato de fechas antes de filtrar
        String textoDesde = txtDesde.getText().trim();
        String textoHasta = txtHasta.getText().trim();

        if (!textoDesde.isEmpty() && parsearFecha(textoDesde) == null) {
            JOptionPane.showMessageDialog(this,
                "Formato de fecha 'Desde' inválido. Use YYYY-MM-DD",
                "Fecha inválida", JOptionPane.WARNING_MESSAGE);
            txtDesde.requestFocus();
            return;
        }
        if (!textoHasta.isEmpty() && parsearFecha(textoHasta) == null) {
            JOptionPane.showMessageDialog(this,
                "Formato de fecha 'Hasta' inválido. Use YYYY-MM-DD",
                "Fecha inválida", JOptionPane.WARNING_MESSAGE);
            txtHasta.requestFocus();
            return;
        }

        Timestamp desde = parsearFecha(textoDesde);
        Timestamp hasta = parsearFecha(textoHasta);
